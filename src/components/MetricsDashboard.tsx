    ...
    <div className="bg-white border border-slate-200 rounded-md shadow-sm">
       <div className="border-b border-slate-100 px-5 py-4 flex items-center justify-between bg-slate-50 rounded-t-md">
          <div className="flex items-center space-x-3">
             <h2 className="text-sm font-semibold">Ringkasan Metrik (Metrics Summary)</h2>
          ...
          <div className="flex items-center space-x-2">
             <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full tracking-wide ${record.integrity_badge === 'valid' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800'}`}>
               Integritas (Integrity): {record.integrity_badge.toUpperCase()}
             </span>
          </div>
       </div>
       <div className="p-5 grid grid-cols-3 gap-4">
          <div className="border border-slate-100 p-4 rounded-md">
             <div className="text-xs text-slate-500 mb-1">Ukuran Pesan (Payload Size)</div>
          ...
          <div className="border border-slate-100 p-4 rounded-md">
             <div className="text-xs text-slate-500 mb-1">Kapasitas Terpakai (Utilization)</div>
             <div className="text-lg font-semibold text-slate-900">{(record.payload_utilization_ratio * 100).toFixed(2)}%</div>
          </div>
          <div className="border border-slate-100 p-4 rounded-md">
             <div className="text-xs text-slate-500 mb-1">Integritas Data (CRC Validation)</div>
             <div className="text-lg font-semibold text-slate-900">{record.crc_valid ? 'Lulus (Valid)' : 'Gagal (Failed)'}</div>
          </div>
       </div>
